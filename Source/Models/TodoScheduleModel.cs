...
namespace Todos.Source.Models
{
    public class TodoScheduleModel : ModelBase
    {
        ...
        public readonly IProperty<string> ClipboardContent;
        ...
        {
            ScheduleDropdown = Add(Variables.Persistent(ResetFactory.FromType(json.Type).DropdownEntry,
                v => json.Type = ResetFactory.FromDropdown(v).Type, json.Persist));
            ...
            LastExecution = Add(Executions.Select(executions =>
                executions.Count > 0 ? executions.Max().WithoutSeconds() : (DateTimeOffset?)null));

            IsDone = Add(TimeService.NewMinute.CombineWith(LastExecution, Reset, LocalTime, Duration, GetIsDone));
            IconTooltip =
                Add(TimeService.NewMinute.CombineWith(LastExecution, Reset, LocalTime, Duration, GetIconTooltip));
            CheckboxTooltip = Add(IsDone.CombineWith(LastExecution, GetCheckboxTooltip));
            ClipboardContent = Add(TimeService.NewMinute.CombineWith(Reset, GetClipboardContent));
        }

        private static string GetClipboardContent(DateTimeOffset now, IReset reset)
        {
            return reset.ClipboardContent(now);
        }

        private static bool GetIsDone(DateTimeOffset now, DateTimeOffset? lastExecution, IReset reset,
            TimeSpan localTime, TimeSpan duration)
        {
            return lastExecution.HasValue && reset.IsDone(now, lastExecution.Value, localTime, duration);
        }

        private static string GetIconTooltip(DateTimeOffset now, DateTimeOffset? lastExecution, IReset reset,
            TimeSpan localTime, TimeSpan duration)
        {
            return reset.IconTooltip(now, lastExecution, localTime, duration);
        }
        ...
        private static string GetCheckboxTooltip(bool isDone, DateTimeOffset? lastExecution)
        {
            return !isDone
                ? null
                : $"Done: {lastExecution?.ToDaysSinceString()}, {lastExecution?.LocalDateTime.ToShortTimeString()}";
        }
        ...
        public void ToggleDone()
        {
            if (!IsDone.Value)
            {
                Executions.Value = Executions.Value.Append(DateTimeOffset.Now.WithoutSeconds()).ToList();
            }
